...
def source_shape(db: sqlite3.Connection, source_id: str) -> dict[str, int]:
    return {
        category: count
        for category, count in db.execute(
            "SELECT category,count(*) FROM meta_entries WHERE source_id=? GROUP BY category",
            (source_id,),
        )
    }
...
def validate_source_shape(
    source: MetaSource, before: dict[str, int], after: dict[str, int]
) -> None:
    """Reject a partial import before it can replace a usable source snapshot."""
    before_total = sum(before.values())
    after_total = sum(after.values())
    minimum_total = max(10, (before_total * 3) // 5)
    if after_total < minimum_total:
        raise RuntimeError(
            f"{source.product} Meta import too small: {after_total} < {minimum_total}"
        )
    for category, old_count in before.items():
        minimum_category = max(1, (old_count * 3) // 5)
        if after.get(category, 0) < minimum_category:
            raise RuntimeError(
                f"{source.product} Meta category {category} too small: "
                f"{after.get(category, 0)} < {minimum_category}"
            )
...
def ensure_meta_schema(db: sqlite3.Connection) -> None:
    db.executescript(
        ...
        CREATE TABLE IF NOT EXISTS meta_sources(
          ...
          precedence INTEGER NOT NULL DEFAULT 0, artifact_id TEXT);
    ...
    if "artifact_id" not in columns:
        db.execute("ALTER TABLE meta_sources ADD COLUMN artifact_id TEXT")
...
def import_meta_source(
    ...
    artifact_id: str | None = None, allow_unclosed: bool = False,
) -> int:
    ...
    db.execute("UPDATE meta_sources SET artifact_id=? WHERE source_id=?", (artifact_id, source.source_id))
    ...
    with zipfile.ZipFile(io.BytesIO(snapshot)) as archive:
        ...
        for name in java_names:
            ...
            for category, line, body, closed in parse_blocks(text):
                ...
                count += 1
    if unclosed_blocks and not allow_unclosed:
        raise RuntimeError(
            f"{source.product} Meta import has {unclosed_blocks} unclosed annotation blocks"
        )
    if unclosed_blocks:
        db.execute(
            "INSERT OR REPLACE INTO metadata(key,value) VALUES(?,?)",
            (f"meta.snapshot.{source.source_id}.incomplete", f"unclosed_blocks={unclosed_blocks}"),
        )
    return count
...
def classify_deltas(db: sqlite3.Connection) -> None:
    ...
    official: dict[tuple[str, str, str], tuple[int, str]] = {}
    for entry_id, category, lowered, object_type, raw_json in db.execute(
        """SELECT entry_id,category,lower(name),object_type,raw_fields_json FROM meta_entries
           WHERE source_id IN ('denizen_official_dev','denizencore_official_master')
           ORDER BY category,lower(name),object_type,source_file,source_line,entry_id"""
    ):
        official.setdefault((category, lowered, object_type), (entry_id, raw_json))
    for entry_id, source_id, category, lowered, object_type, raw_json in db.execute(
        """SELECT entry_id,source_id,category,lower(name),object_type,raw_fields_json
           FROM meta_entries
           ORDER BY source_id,category,lower(name),object_type,source_file,source_line,entry_id"""
    ):
        if source_id == "denizenm_public_master":
            baseline = official.get((category, lowered, object_type))
...
def import_ide(db: sqlite3.Connection, version: str, commit: str, dll: bytes, sharp: bytes) -> int:
    ...
    db.execute(
        ...
        hashlib.sha256(sharp).hexdigest(), "https://github.com/DenizenScript/SharpDenizenTools",
...
def apply_update(
    ...
    with tempfile.TemporaryDirectory(prefix="dcore_update_") as temp:
        ...
        with closing(sqlite3.connect(candidate)) as db:
            ...
            previous_shapes = {
                source.source_id: source_shape(db, source.source_id)
                for source in META_SOURCES
            }
            counts = {}
            for source in META_SOURCES:
                counts[source.product] = import_meta_source(
                    db, source, heads[source.source_id], snapshots[source.source_id]
                )
                validate_source_shape(
                    source, previous_shapes[source.source_id], source_shape(db, source.source_id)
                )
            classify_deltas(db)
