...
Load the pack on vanilla 1.21.8. In a disposable world with commands enabled: /summon minecraft:pig ~ ~ ~ {Tags:["lesson_probe"],NoAI:1b,Glowing:1b}. Keep it visible. Disable with /kill @e[tag=lesson_probe].

Expected (not a screenshot or runtime result): The glowing pig silhouette becomes translucent cyan instead of a thin vanilla outline. Ordinary entities remain unchanged.
