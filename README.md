...
- a hard teaching gate: one bounded fragment or exercise per response, with no one-shot full script by default;
- worked-example fading, user attempt, feedback and transfer checks;
- anti-vibe review based on observable code risk, never alleged authorship;
- lint-policy consistency for DenizenM and documented Reflect boundaries.
...
```text
python tools/build_gpt_bundle.py --output dist/dCore-0.31
```
