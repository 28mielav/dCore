...
def has_unclosed_tags(line: str) -> bool:
    """Count Denizen tags without treating comparison/flag operators as tags.

    A tag opener is '<' followed immediately by a non-whitespace token starter.
    A bare comparison '<' or '>' outside a tag is ignored. This remains a
    structural check, not a parser for Denizen tag semantics.
    """
    depth = 0
